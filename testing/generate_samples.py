...
def print_markdown_paragraph(pdf, text, font_size=9.5, line_height=4.5):
    ...
    # Strip custom artifacts from earlier code that aren't valid markdown or that we handled manually
    # The previous code handled horizontal rules `---`. markdown library handles this natively by creating <hr> tags.
    # The markdown library handles all headers, lists, and bold text natively.
    
    # Convert markdown to HTML
    html = markdown.markdown(text)
    
    # Inject CSS line-height to fix tight font spacing
    # Using 1.5 as a multiplier gives comfortable reading spacing
...
def generate_pdf_page(pdf, image_path, image_name, llm_id, llm_name, style_id, style_name, preds, interpretation):
    ...
    """
    # Sanitize strings to avoid FPDF Helvetica encoding failures
    ...
    # Dynamic styling based on text length to keep layout perfectly balanced and strictly 2 pages per case
    ...
    # Get original image dimensions and aspect ratio to prevent distortion
    try:
        ...
        img_display_h = img_display_w  # Default to square fallback
    
    # Render centered image without stretching
    x_center = (210 - img_display_w) / 2  # A4 width is 210mm
    ...
    # Prepare Big Five Scores string
    scores = []
    ...
    # Shaded, structured Metadata Box (180mm width)
    pdf.set_fill_color(245, 245, 245)
    box_y = pdf.get_y()
    pdf.rect(15, box_y, 180, 25, "F")  # Bounding box filled with light gray
    
    # Row 1: File Gambar & Model Inferensi
    ...
    # Render interpretation text with custom markdown bullet/bold/indent parser
    print_markdown_paragraph(pdf, interpretation, font_size=font_size, line_height=line_height)
    pdf.ln(4)
    
    # 4. Psychologist Likert Validation Form (Always starts on a clean page to prevent mid-table page breaks and layout bugs)
    pdf.add_page()
    ...
    # 5. Load dynamic criteria configurations from metrics_config.json
        ...
        try:
            with open(metrics_path, "r", encoding="utf-8") as f:
                ...
                for idx, m in enumerate(metrics_list, 1):
                    # Sanitize criteria description to look compact
                    desc = m["criteria"].split(". ")[0] + "."  # Take first sentence as description
    ...
    # Append Penilaian Keseluruhan at the end
    ...
    # Draw table rows dynamically calculating height
    ...
    # Visual Box for writing feedback
...
def run_pipeline():
    ...
    if not images:
        print("[!] No images found to process. Please place images in testing/images/ directory.")
        return
    ...
    for img_file in images:
        ...
        for llm in llm_list:
            ...
            for style in style_list:
                ...
                # Check if this combination has already been successfully processed
                # If so, load existing JSON data from disk to support resume without duplicate APIs
                if os.path.exists(json_path):
                    ...
                    try:
                        ...
                        # Compile to master PDF from saved data
                        generate_pdf_page(
                            pdf=master_pdf,
                ...
                try:
                    ...
                    with open(image_path, "rb") as img_f:
                        ...
                        # Increased timeout to 300s (5 minutes) to prevent read timeouts on large models/styles
                    ...
                    # 2. Append page to the unified master PDF document
                    generate_pdf_page(
                        pdf=master_pdf,
                        image_path=image_path,
                        image_name=img_file,
    ...
    # 3. Output the single consolidated PDF report
    if has_pages:
        # Determine output filename based on how many images were processed
        if len(images) == 1:
            # Get the name of the single image processed
